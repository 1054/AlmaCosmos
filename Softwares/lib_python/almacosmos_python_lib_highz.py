...
sys_path_to_add = os.path.dirname(__file__)
...
sys_path_to_add = os.path.dirname(os.path.dirname(__file__))+os.sep+'lib_python_dzliu'
if not sys_path_to_add in sys.path:
    print('Adding sys.path "%s"'%(sys_path_to_add))
    sys.path.append(sys_path_to_add)

from crabtable.CrabTable import CrabTable, CrabTableReadInfo
...
class Highz_Galaxy(object):
    # 
    def __init__(self, ID=-1, SubID=-1, Name="", Field="", Names={}, RA=-1E0, Dec=-1E0, Epoch="J2000", z=0.0, Morphology={}, Photometry={}, Redshifts={}):
        self.ID = int(ID)
        self.RA = float(RA)
        self.Dec = float(Dec)
...
class Highz_Catalogue(object):
    ...
    def open(self, FitsTableFile):
        if FitsTableFile != '':
            ...
            self.FitsTablePointer = CrabTable(FitsTableFile)
    ...
    def col(self, ColName=[], ColSelect=[1], RowSelect=[], CheckExistence=False, ReturnColName=False):
        ...
        if len(RowSelect) > 0:
            if ColCount > 0:
                if RowSingle > 0:
                    if ColSingle > 0:
                        if ReturnColName:
                            return ColOutput[RowSelect[:,None],ColSelect-1], ColHead
                        else:
                            return ColOutput[RowSelect[:,None],ColSelect-1]
                    else:
                        if ReturnColName:
                            return ColOutput[RowSelect[:],ColSelect[0]-1], ColHead
                        else:
                            return ColOutput[RowSelect[:],ColSelect[0]-1]
                else:
                    if ColSingle > 0:
                        if ReturnColName:
                            return ColOutput[RowSelect[0,None],ColSelect-1], ColHead
                        else:
                            return ColOutput[RowSelect[0,None],ColSelect-1]
                    else:
                        if ReturnColName:
                            return ColOutput[RowSelect[0],ColSelect[0]-1], ColHead
                        else:
                            return ColOutput[RowSelect[0],ColSelect[0]-1]
            else:
                if ReturnColName:
                    return numpy.nan, ColHead
                else:
                    return numpy.nan
        # return
        ...
        if OutputFilePath and self.TableHeaders and len(self.TableData)>0:
            ...
            for i in range(len(self.TableHeaders)):
                ...
                if colfmt.find('int64')>=0:
                    ...
                    colfmt = 'A'
                elif colfmt.find('str')>=0:
                    colfmt = colfmt.replace('str','A')
                elif colfmt.find('bool')>=0:
                    colfmt = 'L'
            ...
            if os.path.isfile(OutputFilePath) and not overwrite:
                ...
                hdu.writeto(OutputFilePath, overwrite=overwrite)
    # 
    def object(self, InputIndex=[], ReturnColName=False):
    ...
    # 
    def id(self, InputIndex=[], ReturnColName=False):
        ColList = ['ID','INDEX','id','_id','index','NUMBER','ID_1','id_1','ID_2','id_2','id_.*','ID_.*']
        return self.col(ColName=ColList, ColSelect=1, RowSelect=InputIndex, ReturnColName=ReturnColName)
    # 
    def id_2(self, InputIndex=[], ReturnColName=False):
        ColList = ['ID','INDEX','id','_id','index','NUMBER','ID_1','id_1','ID_2','id_2','id_.*','ID_.*']
        return self.col(ColName=ColList, ColSelect=2, RowSelect=InputIndex, ReturnColName=ReturnColName)
    # 
    def ra(self, InputIndex=[], ReturnColName=False):
        ColList = ['RA','ra','ALPHA_J2000','_ra','RA_1','ra_1','RA_2','ra_2','ra_.*','RA_.*']
        return self.col(ColName=ColList, ColSelect=1, RowSelect=InputIndex, ReturnColName=ReturnColName)
    # 
    # read a second RA/Dec column in the catalog
    def ra_2(self, InputIndex=[], ReturnColName=False):
        ColList = ['RA','ra','ALPHA_J2000','_ra','RA_1','ra_1','RA_2','ra_2','ra_.*','RA_.*']
        return self.col(ColName=ColList, ColSelect=2, RowSelect=InputIndex, ReturnColName=ReturnColName)
    # 
    def dec(self, InputIndex=[], ReturnColName=False):
        ColList = ['DEC','Dec','dec','DELTA_J2000','_dec','DEC_1','Dec_1','dec_1','DEC_2','Dec_2','dec_2','dec_.*','Dec_.*','DEC_.*']
        return self.col(ColName=ColList, ColSelect=1, RowSelect=InputIndex, ReturnColName=ReturnColName)
    # 
    # read a second RA/Dec column in the catalog
    def dec_2(self, InputIndex=[], ReturnColName=False):
        ColList = ['DEC','Dec','dec','DELTA_J2000','_dec','DEC_1','Dec_1','dec_1','DEC_2','Dec_2','dec_2','dec_.*','Dec_.*','DEC_.*']
        return self.col(ColName=ColList, ColSelect=2, RowSelect=InputIndex, ReturnColName=ReturnColName)
    # 
    def zphot(self, InputIndex=[], ReturnColName=False):
        ColList = ['PHOTOZ','zphot','photo-z','z_phot','zPDF','ZPDF','ZPDF_.*','ZML','ZML_.*','z','Laigle_Z_MED','Laigle_ZPDF']
        return self.col(ColName=ColList, ColSelect=1, RowSelect=InputIndex, ReturnColName=ReturnColName)
    # 
    def zspec(self, InputIndex=[], ReturnColName=False):
    ...
    def morph(self, InputIndex=[], ColSelect=1, ReturnColName=False):
        ...
        if ReturnColName:
            ...
            return ColVal_maj, ColVal_min, ColVal_pa
    # 
...
class Highz_Image(object):
    ...
    def __init__(self, FitsImageFile):
        ...
        if not os.path.isfile(self.FitsImageFile):
            ...
            sys.exit()
        # 
        # read FitsImageFile with dzliu python class 'CrabImage'
        self.FitsImagePointer = CrabImage(self.FitsImageFile)
    ...
    def plot(self, zoom_rect=[], zoom_center=[], zoom_size=[]):
        ...
        if len(zoom_rect) != 4 and len(zoom_center) == 2 and len(zoom_size) >= 1:
            ...
            zoom_rect = [ numpy.round(zoom_center[0]-(zoom_size[0]/2.0)).astype(int), 
                          numpy.round(zoom_center[0]+(zoom_size[0]/2.0)).astype(int), 
                          numpy.round(zoom_center[1]-(zoom_size[1]/2.0)).astype(int), 
                          numpy.round(zoom_center[1]+(zoom_size[1]/2.0)).astype(int) ]
        ...
        if not os.path.isfile(self.FitsImageFile+'.pixel.statistics.txt'):
            os.system('%s "%s"'%('almacosmos_get_fits_image_pixel_histogram.py', self.FitsImageFile))

...
#              2019-05-10 disabled ngmax, now using official bdsf from pip install.
# 
...
import scipy
import bdsf
# requires `brew install boost --with-python`
# requires `brew install boost boost-python`
#          `sudo pip install bdsf`
...
if len(sys.argv) <= 1:
    ...
    print('    ')
    print('    AlmaCosmos_Photometry_Blind_Extraction_PyBDSM.py "ALMA_Image_List.txt" -thresh_pix 3.5           # (new parameter to tune, default 3.5)')
    print('    AlmaCosmos_Photometry_Blind_Extraction_PyBDSM.py "ALMA_Image_List.txt" -thresh_rms 3.5           # (new parameter to tune, default 3.5)')
    print('    ')
    sys.exit()
    ...
    if sys.argv[i].endswith('.fits') or sys.argv[i].endswith('.FITS') or \
        ...
        if temp_arg_str == '-start':
            ...
            if i+1 <= len(sys.argv)-1:
                ...
                i = i + 1
        #<20190510>#elif temp_arg_str == '-max-gaussian-number' or \
        #<20190510>#    temp_arg_str == '-ngmax' or \
        #<20190510>#    temp_arg_str == '-number-gaussian' or \
        #<20190510>#    temp_arg_str == '-numb-gauss' or \
        #<20190510>#    temp_arg_str == '-max-numb-gaussian' or \
        #<20190510>#    temp_arg_str == '-max-numb-gauss' or \
        #<20190510>#    temp_arg_str == '-max-gaussian-numb':
        #<20190510>#    if i+1 <= len(sys.argv)-1:
        #<20190510>#        input_ini_gausfit = 'ngmax'+' '+(sys.argv[i+1])
        #<20190510>#        input_peak_fit = False
        #<20190510>#        print('Setting ini_gausfit to %s'%(input_ini_gausfit))
        #<20190510>#        i = i + 1

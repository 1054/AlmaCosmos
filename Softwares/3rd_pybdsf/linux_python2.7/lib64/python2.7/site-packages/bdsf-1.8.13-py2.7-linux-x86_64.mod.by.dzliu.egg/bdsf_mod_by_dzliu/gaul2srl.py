...
#################################################################################################
    ...
    def process_single_gaussian(self, img, g_list, src_index, code):
        ...
        posn_pix_centroid = posn_pix_max = [g.centre_pix, g.centre_pixE] #<20171109># Added by dzliu
        ...
        source_prop = list([code, total_flux, peak_flux_centroid, peak_flux_max, aper_flux, \
             posn_sky_centroid, \
             posn_sky_max, \
             posn_pix_centroid, \
             posn_pix_max, \
             size_sky, size_sky_uncorr, deconv_size_sky, deconv_size_sky_uncorr, bbox, ngaus, island_id, gaussians])
...
##################################################################################################
    ...
    def process_Multiple(self, img, g_sublist, mask, src_index, isrc, subim, isl, delc, subn, subm):
        ...
        try:
            ...
            posn_pix_centroid_dzliu = [mompara[1]+delc[0], mompara[2]+delc[1]]
            posn_pix_max_dzliu = posn.tolist()
        ...
        posn_pix_centroidE_dzliu = [sqrt(mompara1E**2 + errors[1]**2),\
                                    sqrt(mompara2E**2 + errors[2]**2)] #<20171109># Added by dzliu
        posn_pix_maxE_dzliu = posn_pix_centroidE_dzliu #<20171109># Added by dzliu
        ...
        source_prop = list(['M', [tot, totE], [s_peak, isl.rms], [maxpeak, isl.rms], [aper_flux, aper_fluxE], \
                      [[sra, sdec], [sraE, sdecE]], \
                      [[mra, mdec], [sraE, sdecE]], \
                      [posn_pix_centroid_dzliu, posn_pix_centroidE_dzliu], \
                      [posn_pix_max_dzliu, posn_pix_maxE_dzliu], \
                      [size_sky, size_skyE], \
                      [size_sky_uncorr, size_skyE], \
...
class Source(object):
    ...
    def __init__(self, img, sourceprop):
        ...
        code, total_flux, peak_flux_centroid, peak_flux_max, aper_flux, \
                     posn_sky_centroid, \
                     posn_sky_max, \
                     posn_pix_centroid, \
                     posn_pix_max, \
                     size_sky, size_sky_uncorr, deconv_size_sky, \
        ...
        self.posn_pix_centroid, self.posn_pix_centroidE = posn_pix_centroid #<20171109># Added by dzliu
        self.posn_pix_max, self.posn_pix_maxE = posn_pix_max #<20171109># Added by dzliu

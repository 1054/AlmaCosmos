...
class Op_gausfit(Op):
    ...
    def fit_island(self, isl, opts, img, ngmax=None, ffimg=None, ini_gausfit=None):
        ...
        #<20171108><DZLIU># -------------------------------------------------------------------------
        #<20171108><DZLIU># Modified
        #<20171108><DZLIU>#     if ini_gausfit not in ['default', 'simple', 'nobeam']:
        #<20171108><DZLIU># to
        #<20171108><DZLIU>#     ini_gausfit = 'default'
        #<20171108><DZLIU># and 
        #<20171108><DZLIU># added a lot of 'ini_gausfit' 'ngmax *' stuff
        if ini_gausfit not in ['default', 'simple', 'nobeam'] and not ini_gausfit.startswith('ngmax'):
            ini_gausfit = 'default'
        if verbose:
            print("Reading argument ini_gausfit " + ini_gausfit + ' / ' + opts.ini_gausfit)
        if ini_gausfit.startswith('ngmax') and ngmax is None:
            if len(ini_gausfit.split())>1:
                ngmax = long((ini_gausfit.split())[1]) #<20171108># added by dzliu
                print("Setting maximum number of Gaussians to " + str(ngmax))
        #<20171108><DZLIU># -------------------------------------------------------------------------
        ...
        while iter < 5:
            ...
            if fitok and len(fgaul) == 0:
                break
        #<20171109><DZLIU># -------------------------------------------------------------------------------------------------------------------
        #<20171109><DZLIU># Modified
        #<20171109><DZLIU>#     if (not fitok or len(gaul) == 0) and ini_gausfit != 'simple':
        #<20171109><DZLIU># to
        #<20171109><DZLIU>#     if (not fitok or len(gaul) == 0) and ini_gausfit != 'simple' and not ini_gausfit.startswith('ngmax'):
        #<20171109><DZLIU># and
        #<20171109><DZLIU># added if verbose: print
        if verbose:
            print("fitok = %s"%(fitok))
            print("len(gaul) = %s"%(len(gaul)))
        if (not fitok or len(gaul) == 0) and ini_gausfit != 'simple' and not ini_gausfit.startswith('ngmax'):
            # If fits using default or nobeam methods did not work,
            # try using simple instead
            gaul = []
            iter = 0
            ng1 = 0
            #<20171109><DZLIU># ngmax = 25 #---------
            if not ini_gausfit.startswith('ngmax'):
                ngmax = 25
            #<20171109><DZLIU># ---------------------
            while iter < 5:
               iter += 1
        ...
        if (not fitok or len(gaul) == 0) and N.sum(~sm_isl) >= img.minpix_isl:
            ...
            if verbose:
                print("Previous fitting is not good, shrink the island a little and try again")
        ...
        if (not fitok or len(gaul) == 0) and N.sum(~lg_isl) >= img.minpix_isl:
            ...
            if verbose:
                print("Previous fitting is not good, expand the island a little and try again")
        ...
        if not fitok or len(gaul) == 0:
            ...
            if verbose:
                print("Previous fitting is not good, try to use moment analysis")
        ...
        if verbose:
            ...
            #<20171109><DZLIU># Added the following printed message --------------------------------------------------------------------------------
            for fgaul_flag, fgaul_g in fgaul:
                print '    flag %s -- flag bits %s -- g (A, x1, x2, s1, s2, th) %s' % (fgaul_flag, bin(fgaul_flag), fgaul_g)
                if fgaul_flag == 64:
                    print '    flag %s -- flag bits %s -- beam %s' % (fgaul_flag, bin(fgaul_flag), beam)
                    print '    flag %s -- flag bits %s -- flag_maxsize_bm %s' % (fgaul_flag, bin(fgaul_flag), opts.flag_maxsize_bm)
                    print '    flag %s -- flag bits %s -- gaus area / beam %s' % (fgaul_flag, bin(fgaul_flag), (g[3]*g[4])/(beam[0]*beam[1]))
            #<20171109><DZLIU># --------------------------------------------------------------------------------------------------------------------
        return gaul, fgaul
...
class Gaussian(object):
    ...
    def __init__(self, img, gaussian, isl_idx, g_idx, flag=0):
        ...
        if flag == 0:
            ...
            gaus_dc2 = func.deconv(bm_pix, size)
            print("debug by dzliu: island id %s, total flux %s, peak flux %s, "%(isl_idx, tot, p[0]))
            print("                              convolved size %s %s %s [pix pix deg], beam %s %s %s [pix pix deg]"%(size[0], size[1], size[2], bm_pix[0], bm_pix[1], bm_pix[2]))
            print("                              MIRIAD deconvolved size %s %s %s [pix pix deg], "%(gaus_dc[0], gaus_dc[1], gaus_dc[2]))
            print("                              AIPS deconvolved size %s %s %s [pix pix deg]"%(gaus_dc2[0], gaus_dc2[1], gaus_dc2[2]))

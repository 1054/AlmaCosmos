...
def process(img, **kwargs):
    ...
    #<20180310><dzliu># 
    #<20180310><dzliu># change the default log file to be under current directory instead of image file directory if we do not have write permission
    #log = img.opts.filename + '.pybdsf.log'
    import os
    if os.access(os.path.dirname(img.opts.filename), os.W_OK):
        log = img.opts.filename + '.pybdsf.log'
    else:
        logfilepath2 = img.opts.filename
        log = logfilepath2.replace(os.sep,'.') + '.pybdsf.log'
    #<20180310><dzliu># 
    #<20180310><dzliu># 
    img.log = ''

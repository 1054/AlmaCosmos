...
py_version="3.7" # PyBDSF only works with Python 2.7 for now, 
...
if [[ ! -d "${py_prefix}/lib/python$py_version/site-packages/pyfits" ]]; then
    # pyfits pywcs backports.shutil_get_terminal_size
    # astropy backports.shutil_get_terminal_size
    if [[ $(type pip-$py_version 2>/dev/null | wc -l) -gt 0 ]]; then
        echo "pip-$py_version install --ignore-installed --prefix=\"${py_prefix}\" numpy scipy astropy backports.shutil_get_terminal_size"
        pip-$py_version install --ignore-installed --prefix="${py_prefix}" numpy scipy astropy backports.shutil_get_terminal_size
...
if [[ 1 == 1 ]]; then
    ...
    if [[ $(uname) == "Darwin" ]]; then
        ...
        # 
        if [[ ! -f "${py_prefix}/lib/libboost_python37-mt.dylib" ]]; then
            echo "cp /usr/local/opt/boost-python3/lib/libboost_python37-mt.dylib ${py_prefix}/lib/"
            cp /usr/local/opt/boost-python3/lib/libboost_python37-mt.dylib ${py_prefix}/lib/
            echo "cp ${py_prefix}/lib/libboost_python37-mt.dylib ${py_prefix}/lib/libboost_python3-mt.dylib"
            cp ${py_prefix}/lib/libboost_python37-mt.dylib ${py_prefix}/lib/libboost_python3-mt.dylib
            # 
            #chmod +w "${py_prefix}/lib/libboost_python3-mt.dylib"
            #install_name_tool -id "@loader_path/../../../../libboost_python3-mt.dylib" "${py_prefix}/lib/libboost_python3-mt.dylib"
            #otool -L "${py_prefix}/lib/libboost_python3-mt.dylib"
        fi
        if [[ ! -f "${py_prefix}/lib/libboost_numpy37-mt.dylib" ]]; then
            echo "cp /usr/local/opt/boost-python3/lib/libboost_numpy37-mt.dylib ${py_prefix}/lib/"
            cp /usr/local/opt/boost-python3/lib/libboost_numpy37-mt.dylib ${py_prefix}/lib/
            echo "cp ${py_prefix}/lib/libboost_numpy37-mt.dylib ${py_prefix}/lib/libboost_numpy3-mt.dylib"
            cp ${py_prefix}/lib/libboost_numpy37-mt.dylib ${py_prefix}/lib/libboost_numpy3-mt.dylib
        fi
        # 
        if [[ ! -f "${py_prefix}/lib/libgfortran.3.dylib" ]]; then
        ...
        #    echo "cp /opt/local/lib/gcc8/libgfortran.a ${py_prefix}/lib/"
        #    cp /opt/local/lib/gcc8/libgfortran.a ${py_prefix}/lib/
        #fi
        # 
        if [[ ! -d "include" ]]; then
            mkdir "include"
            echo "cp -r /usr/local/opt/boost/include/boost include/"
            cp -r /usr/local/opt/boost/include/boost include/
        fi
        # 
        ...
        #                 build/temp.macosx-*/src/c++/Fitter_dn2g.o \
        #                 build/temp.macosx-*/src/c++/Fitter_dnsg.o \
        #                 build/temp.macosx-*/src/c++/Fitter_lmder.o \
        #                 build/temp.macosx-*/src/c++/MGFunction1.o \
        #                 build/temp.macosx-*/src/c++/MGFunction2.o \
        #                 build/temp.macosx-*/src/c++/cbdsm_main.o \
        #                 build/temp.macosx-*/src/c++/stat.o \
        #                 build/temp.macosx-*/src/c++/num_util/num_util.o \
        #                 -Lsrc/minpack \
        #                 -Lsrc/port3 \
        #                 -L../${os_system}_python${py_version}/lib \
        #                 -lminpack \
        #                 -lport3 \
        #                 -lgfortran \
        #                 -lboost_numpy37-mt \
        #                 -lboost_python37-mt \
        #                 -o "bdsf/_cbdsm.cpython-37m-darwin.so"
        #                 #-o "bdsf/_cbdsm.so"
        # 
        # Check 
        # 
...
# 
if [[ $(uname) == "Darwin" ]]; then
    echo "otool -L \"${py_outdir}/bdsf/_cbdsm.cpython-37m-darwin.so\""
    otool -L "${py_outdir}/bdsf/_cbdsm.cpython-37m-darwin.so"
fi
#python -c "from ctypes import *; CDLL('_cbdsm.cpython-37m-darwin.so')"

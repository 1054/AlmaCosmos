...
if not os.path.isdir('output_PyBDSM_dzliu'):
    print('Error! "output_PyBDSM_dzliu" was not found under current directory!')
    sys.exit()
...
# 
alma_image_list = []
with open('output_PyBDSM_dzliu/list_for_pyBDSM.txt','r') as fp:
...
def read_MC_sim_recovery(alma_image_name, output=None):
    ...
    recovered_dir = 'output_PyBDSM_dzliu' + os.sep + alma_image_name
    if os.path.isfile('statistics_PyBDSM_dzliu/done_output_sim_data_table_%s'%(alma_image_name)):
    ...
    has_print_lines = 0
    output_data_table = 'statistics_PyBDSM_dzliu/output_sim_data_table_%s.txt'%(alma_image_name)
    ...
    # 
    ofs.close()
    os.system('date +"%%Y-%%m-%%d %%H:%%M:%%S %%Z" > %s'%('statistics_PyBDSM_dzliu/done_output_sim_data_table_%s'%(alma_image_name)))

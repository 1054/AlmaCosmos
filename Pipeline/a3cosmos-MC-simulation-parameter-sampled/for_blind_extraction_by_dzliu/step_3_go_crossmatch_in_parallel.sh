...
if [[ ! -d statistics_PyBDSM_dzliu ]]; then mkdir statistics_PyBDSM_dzliu; fi

if [[ -f "statistics_PyBDSM_dzliu/list_of_commands.txt" ]]; then rm "statistics_PyBDSM_dzliu/list_of_commands.txt"; fi

number_of_sources=$(cat output_PyBDSM_dzilu/list_for_pyBDSM.txt | wc -l)

for (( i=1; i<=$number_of_sources; i++ )); do
    
    echo $(dirname "${BASH_SOURCE[0]}")/step_3_go_crossmatch.py "$i" >> "statistics_PyBDSM_dzliu/list_of_commands.txt"
    
done

echo ""
echo "Then, please run"
echo "  almacosmos_cmd_run_in_parallel \"statistics_PyBDSM_dzliu/list_of_commands.txt\""
echo ""

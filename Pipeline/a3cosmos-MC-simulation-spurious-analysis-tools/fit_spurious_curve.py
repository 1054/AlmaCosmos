...
datatable = asciitable.read('datatable_spurious_fraction_cumulative.txt')
...
os.system('echo "# $(date +\"%%Y-%%m-%%d %%H:%%M:%%S %%Z\")" > %s'%('function_spurious_fraction_cumulative.txt'))

os.system('echo "set a = %0.20e" >> %s'%(a, 'function_spurious_fraction_cumulative.txt'))
os.system('echo "set b = %0.20e" >> %s'%(b, 'function_spurious_fraction_cumulative.txt'))
os.system('echo "set c = %0.20e" >> %s'%(c, 'function_spurious_fraction_cumulative.txt'))
os.system('echo "set d = %0.20e" >> %s'%(d, 'function_spurious_fraction_cumulative.txt'))
os.system('echo "set y_fit = d - d / (1 + a*exp(-(b*x+c)))" >> %s'%('function_spurious_fraction_cumulative.txt'))

x_fit = numpy.linspace(numpy.min(x_in), numpy.max(x_in), 1000)
y_fit = d - d / (1 + a*numpy.exp(-(b*x_fit+c)))

asciitable.write({'x_fit': x_fit, 'y_fit': y_fit}, 'function_spurious_fraction_cumulative.xyfit.txt', Writer=asciitable.FixedWidthTwoLine, overwrite=True)

...
# set color
color_list = ['#1e90ff', 'orange', 'magenta', '#222222']


# set output name
output_name = 'Plot_inverted_image_fitting_spurious_curve_in_bins_of_alma_band.pdf'
...
# set and loop alma_band_range0 alma_band_range1
alma_band_str_list = ['3', '5', '6', '7']
for i in range(len(alma_band_str_list)):
    data_dir = 'output_spurious_analysis_bin_by_alma_band_%s'%(alma_band_str_list[i])
    print('Reading from "%s"'%(data_dir))
    # Read data
    ...
    # plot 
    ax.plot(differential_curve['snr'], differential_curve['spur'], color=color_list[i], marker='o', markersize=3, linestyle='none', label='ALMA band %s'%(alma_band_str_list[i]), zorder=9)
    ...
    # 
    print('Number of sources with SNRpeak>5: original image: %d; inverted image: %d'%(numpy.sum(differential_curve['aa'][differential_curve['snr']>=5.0]), numpy.sum(differential_curve['bb'][differential_curve['snr']>=5.0]) ) )
# 
...
if os.getcwd().find('blind')>=0 or os.getcwd().find('Blind_Extraction')>=0:
    ax.text(0.98, 0.58, 'Blind Extraction', transform=ax.transAxes, va='center', ha='right', fontsize=13.5)
elif os.getcwd().find('GALFIT')>=0 or os.getcwd().find('Prior_Fitting')>=0:
    ax.text(0.98, 0.58, 'Prior Fitting', transform=ax.transAxes, va='center', ha='right', fontsize=13.5)

...
#__DOCUMENTATION__      Multi-wavelength_Plots/SED_fitting_magphys_photoz_with_upper_limits_with_24um_with_APER3/stage_2_read_results/
#__DOCUMENTATION__  
...
for (( i=0; i<${#source_list[@]}; i++ )); do
    ...
    if [[ ! -f "$Input_Dir/ID_${source_name}/${SED_fitting_Type}/magphys_fitting/fit_1_with_datatable_photometry_magphys/best-fit.param" ]] || [[ $Do_Overwrite -eq 1 ]]; then
        ...
        if [[ ! -f "1_crop.pdf" ]] || [[ $Do_Overwrite -ge 2 ]]; then
            ~/Cloud/Github/Crab.Toolkit.michi2/bin/bin_magphys/magphys_highz/plot_sed_highz_photoz.bash > /dev/null 2>&1
        fi

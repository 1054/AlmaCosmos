...
if len(sys.argv) <= 1:
    print('Usage: almacosmos_apply_simu_corr_fbias2_via_spline_table.py catalog.txt')
    sys.exit()
...
# 
# read spline_table_fbias2.json which is the output of 'almacosmos_fit_simu_corr_fbias2_via_spline.py'
handle = open('spline_table_fbias2.json', 'r')
...
#                    'datatable_applying_correction_fbias2_via_spline_table.txt', Writer=asciitable.FixedWidth, delimiter=" ", 
#                    names=['x1', 'x2', 'array_extrapolated', 'array_interpolated', 'array_mask'], overwrite=True)
#
#print('Output to "datatable_applying_correction_fbias2_via_spline_table.txt"!')
...
# combined fbias2
# 
fbias2 = copy(array_interpolated)
fbias2[array_mask] = array_extrapolated[array_mask] # or 0.0?
fbias2[data_mask] = numpy.nan

x2[x2mask_upper] = x2backup[x2mask_upper]

#asciitable.write(numpy.column_stack((fbias2, x1, x2)), 
#                    'datatable_applying_correction_fbias2.txt', Writer=asciitable.FixedWidth, delimiter=" ", bookend=True, 
#                    names=['fbias2','x1','x2'], overwrite=True)
#os.system('sed -i.bak -e "1s/^ /#/" "datatable_applying_correction_fbias2.txt"')
#
#print('Output to "datatable_applying_correction_fbias2.txt"!')
...
# 
# final corrected fbias2
# 
S_out_uncorr = data_S_out
S_out_corr = S_out_uncorr + (fbias2 * data_noise)
S_out_corr[data_mask] = numpy.nan

asciitable.write(numpy.column_stack((S_out_corr, S_out_uncorr, fbias2, x2, x1)), 
                    'datatable_applied_correction_fbias2.txt', Writer=asciitable.FixedWidth, delimiter=" ", bookend=True, 
                    names=['S_out_corr','S_out_uncorr','fbias2','x2','x1'], overwrite=True)
os.system('sed -i.bak -e "1s/^ /#/" "datatable_applied_correction_fbias2.txt"')

print('Output to "datatable_applied_correction_fbias2.txt"!')

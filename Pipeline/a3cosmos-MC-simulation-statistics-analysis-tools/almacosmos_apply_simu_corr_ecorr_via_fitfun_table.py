...
if len(sys.argv) <= 1:
    print('Usage: almacosmos_apply_simu_corr_fbias_via_fitfun_table.py catalog.txt')
    sys.exit()
...
#                    'datatable_applying_correction_ecorr_via_fitfun_table.txt', Writer=asciitable.FixedWidth, delimiter=" ", 
#                    names=['x1', 'x2', 'array_extrapolated', 'array_interpolated', 'array_mask'], overwrite=True)
#
#print('Output to "datatable_applying_correction_ecorr_via_fitfun_table.txt"!')
...
asciitable.write(numpy.column_stack((e_S_out_corr, e_S_out_uncorr, ecorr, x2, x1, data_S_out, data_noise)), 
                    'datatable_applied_correction_ecorr.txt', Writer=asciitable.FixedWidth, delimiter=" ", bookend=True, 
                    names=['e_S_out_corr','e_S_out_uncorr','ecorr','x2','x1','S_out','noise'], overwrite=True)

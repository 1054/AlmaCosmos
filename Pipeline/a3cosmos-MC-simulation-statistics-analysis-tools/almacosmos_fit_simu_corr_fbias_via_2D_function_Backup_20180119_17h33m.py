...
def M_model(x1, x2, y_obs, y_err):
    # 
    # priors
    a0 = pymc.Uniform('a0', -2.0, +0.0, value = -2.0)
    a1 = pymc.Uniform('a1', -2.0, +0.0, value = -2.0)
    k1 = pymc.Uniform('k1', +0.0, +2.0, value = +0.0)
    n1 = pymc.Uniform('n1', +0.0, +2.0, value = +0.0)
    a2 = pymc.Uniform('a2', -2.0, +2.0, value = -2.0)
    k2 = pymc.Uniform('k2', +0.0, +2.0, value = +0.0)
    n2 = pymc.Uniform('n2', +0.0, +2.0, value = +0.0)
    # 
    # M_func = pymc.deterministic(*kwargs)
    @pymc.deterministic(plot=False)
    def M_func(x1=x1, x2=x2, a0=a0, a1=a1, k1=k1, n1=n1, a2=a2, k2=k2, n2=n2):
        return a0 * (numpy.exp(a1*numpy.log10((x1/k1)**n1))) * (numpy.exp(a2*((x2/k2)**n2)))
    # likelihood

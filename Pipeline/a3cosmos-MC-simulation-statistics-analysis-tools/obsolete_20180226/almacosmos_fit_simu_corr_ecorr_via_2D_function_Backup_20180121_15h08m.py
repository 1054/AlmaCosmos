...
column_ecorr = 'cell_noi_scatter' # 'cell_rel_scatter_68'
...
x1 = data_x1
x2 = data_x2
y_obs = y_ecorr  # 1/y_ecorr
y_err = numpy.sqrt(1.0/data_cell_size) * y_obs * x2 # <TODO> assign larger errors to larger x2
...
col_names = ['x1 (S_peak/rms_noise)','x2 (Maj_convol/Maj_beam)','1/scatter of (S_in-S_out)/S_in','1 / cell size']
...
def my_func((x1,x2), a1, k1, n1, a2, k2, n2): 
    ...
    return log_of_one_over_scatter


# 
# Initial guess
# 
initial_guess = (1.0, 0.5, 5.0, 0.0, 2.0, 2.0)
bound_range = None
...
def M_model(x1, x2, y_obs_log, y_err_log):
    # 
    # priors
    a1 = pymc.Uniform('a1',   +0.0,  +10.0, value = +0.0)
    k1 = pymc.Uniform('k1',   +0.0,   +1.0, value = +0.3)
    n1 = pymc.Uniform('n1',   +0.0,  +50.0, value = +5.0)
    a2 = pymc.Uniform('a2',   -0.5,   +0.5, value = +0.0)
    k2 = pymc.Uniform('k2',   +0.0,  +10.0, value = +3.0)
    n2 = pymc.Uniform('n2',   -3.0,   +3.0, value = +0.0)
    # 
...
for i2 in range(n2):
    for i1 in range(n1):
        ...
        ax.set_ylim([0.001,1000.0]) # we plot y as 
        ax.set_xscale('log')
        ax.set_yscale('log')
        # 
        # plot observed data
...
os.system('echo "set log_of_one_over_scatter = lg(x1) - (a1 * exp(-lg(x1/k1 + x2/k2)**n1)) + a2"    >> best_fit_function_ecorr.sm')
os.system('echo "set y_fit = log_of_one_over_scatter"                                               >> best_fit_function_ecorr.sm')
...
os.system('echo "    log_of_one_over_scatter = numpy.log10(x1) - (a1 * numpy.exp(-numpy.log10(x1/k1 + x2/k2)**n1)) + a2"    >> best_fit_function_ecorr_via_CurveFit.py')
os.system('echo "    return log_of_one_over_scatter"                                                                        >> best_fit_function_ecorr_via_CurveFit.py')
...
os.system('echo "    log_of_one_over_scatter = numpy.log10(x1) - (a1 * numpy.exp(-numpy.log10(x1/k1 + x2/k2)**n1)) + a2"    >> best_fit_function_ecorr_via_MCMC.py')
os.system('echo "    return log_of_one_over_scatter"                                                                        >> best_fit_function_ecorr_via_MCMC.py')
...
# 
os.system('cp best_fit_function_ecorr_via_MCMC.py best_fit_function_ecorr.py')

...
topcat -stilts tpipe in="datatable_CrossMatched_all_entries.fits.gz" ifmt=fits \
                    cmd="select \"(flag_matched)\"" \
                    cmd="select \"(Galfit_reduced_chi_square_from_rec_cat<20)\"" \
                    cmd="select \"(Pb_corr_pb_image_from_rec_cat>0)\"" \
                    cmd="select \"(Pb_corr_pb_image_from_rec_cat<5)\"" \
                    cmd="select \"(Pb_corr_equation_from_rec_cat<5)\"" \

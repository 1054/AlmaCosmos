...
if __name__ == '__main__':
    ...
    if output_format == 'ascii':
        tb.write(output_catalog_name+'_with_pb_attenu.txt', format='ascii.fixed_width')
        with open(output_catalog_name+'_with_pb_attenu.txt', 'r+') as fp:
            fp.seek(0)
            fp.write('#')
        print('Output to "%s"'%(output_catalog_name+'_with_pb_attenu.txt'))
    else:
        tb.write(output_catalog_name+'_with_pb_attenu.fits', format='fits')
        print('Output to "%s"'%(output_catalog_name+'_with_pb_attenu.fits'))

...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
        print('Usage: ')
        print('    a_dzliu_code_calc_primary_beam_attenuation.py primary_beam_dist_arcsec sky_frequency_GHz')
        print('')
        sys.exit()
    
    # read user input
    primary_beam_dist = float(sys.argv[1])
    sky_frequency = float(sys.argv[2])
    
    calc_primary_beam_attenuation(primary_beam_dist, sky_frequency, verbose=True)
